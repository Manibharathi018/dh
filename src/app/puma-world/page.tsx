...
export default function PumaWorldPage() {
  const [page, setPage] = useState(0);

  // Fetch products by brand PUMA
  const { data: productsData, isLoading } = useQuery({
    queryKey: ["products-puma", page],
    queryFn: () => productService.filterProducts({ brand: "PUMA" }, page, 12),
  });
    ...
    <div className="bg-white min-h-screen pt-12 pb-24">
      <div className="container mx-auto px-6">
        ...
        <div className="border-b border-gray-100 pb-8 mb-12">
          <h1 className="text-4xl md:text-5xl font-heading font-bold tracking-tight mb-4 text-[#8B6914] uppercase">
            PUMA World
          </h1>
          <p className="text-muted-foreground max-w-xl">
            Explore our premium selection of PUMA products. Sourced directly, offering the best performance and style.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-gray-50/50 border border-dashed border-gray-200">
            <p className="text-muted-foreground mb-4 font-heading text-lg">No PUMA products found right now.</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-8 sm:gap-x-8 sm:gap-y-16">
            {products.map((product, index) => (
              <Link
                key={product.id || `puma-prod-${index}`}
              ...
              >
                ...
                <div className="relative aspect-[3/4] overflow-hidden bg-gray-50 mb-6">
                  <Image
                    src={product.imageUrls?.[0] || "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?q=80&w=1620&auto=format&fit=crop"}
                    alt={product.name}
                    fill
