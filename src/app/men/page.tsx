  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-10 md:gap-x-8 md:gap-y-16">
      {products.map((product: Product, index: number) => {
        ...
        return (
          <Link
            key={product.id || `mens-prod-${index}`}
...
function MensPageContent() {
  ...
  const rootCategory = allCategories.find(c => c.slug?.toLowerCase() === "men");
  ...
  if (!rootCategory) {
    return (
      <div className="container mx-auto px-6 py-32 text-center">
        ...
        <p className="text-muted-foreground mb-8">The "Men" collection is currently unavailable.</p>
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="text-center mb-10">
          ...
          {activeCategory && (
            ...
            >
              <ArrowLeft className="w-4 h-4 mr-1.5" /> Back to all Men's Categories
            </button>
          )}
        </div>
        ...
        <div className="border-t border-gray-100 pt-16">
          <h2 className="text-2xl md:text-3xl font-heading font-medium text-center uppercase tracking-widest mb-10">
            {activeCategory ? activeCategory : "ALL MENS PRODUCTS"}
          </h2>
...
export default function MensPage() {
  return (
    <Suspense fallback={<div className="container mx-auto px-6 py-32 text-center text-muted-foreground">Loading Men's Collection...</div>}>
      <MensPageContent />
    </Suspense>
  );
}
