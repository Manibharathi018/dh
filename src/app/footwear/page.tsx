  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-10 md:gap-x-8 md:gap-y-16">
      {products.map((product: Product, index: number) => {
        ...
        return (
          <Link
            key={product.id || `footwear-prod-${index}`}
          ...
          >
            ...
            <div className="flex flex-col flex-grow">
              <p className="text-[10px] tracking-widest text-muted-foreground uppercase mb-1">
                {product.brand || (product.name?.toLowerCase().includes("puma") ? "PUMA" : "DFO")}
              </p>
...
function FootwearPageContent() {
  ...
  const brandParam = searchParams.get("brand") ? searchParams.get("brand")!.toLowerCase() : null;
  ...
  const rootCategory = allCategories.find(c => c.slug?.toLowerCase() === "footwear");
  ...
  if (!rootCategory) {
    return (
      <div className="container mx-auto px-6 py-32 text-center">
        ...
        <p className="text-muted-foreground mb-8">The "Footwear" collection is currently unavailable.</p>
  ...
  // Apply Brand Filter (e.g. only PUMA)
  if (brandParam) {
    productsToDisplay = productsToDisplay.filter((p: Product) => {
      const nameMatch = p.name ? p.name.toLowerCase().includes(brandParam) : false;
      const brandMatch = p.brand ? p.brand.toLowerCase().includes(brandParam) : false;
      const descMatch = p.description ? p.description.toLowerCase().includes(brandParam) : false;
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="text-center mb-10">
          <h1 className="text-3xl md:text-5xl font-heading font-medium tracking-wide uppercase mb-2">
            {brandParam ? `${brandParam.toUpperCase()} FOOTWEAR` : "FOOTWEAR"}
          </h1>
          <p className="text-xs md:text-sm text-muted-foreground uppercase tracking-widest mt-1">
            {brandParam ? "Exclusive Factory-Direct Silhouettes" : "Performance & Lifestyle Shoes"}
          </p>

          {/* Active Brand Filter Tag with Reset */}
          {brandParam && (
            <div className="mt-4 inline-flex items-center gap-2 bg-neutral-100 px-3 py-1.5 rounded-full text-xs font-medium text-neutral-800">
              <span>Brand: <strong className="uppercase">{brandParam}</strong></span>
              <button
          ...
          {activeCategory && (
            <button
              ...
              className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground transition-colors mt-4 block mx-auto"
            >
              <ArrowLeft className="w-4 h-4 mr-1.5" /> Back to all Footwear Categories
            </button>
          )}
        </div>

        {!activeCategory && !brandParam && subcategories.length > 0 && (
        ...
        <div className="border-t border-gray-100 pt-16">
          <h2 className="text-2xl md:text-3xl font-heading font-medium text-center uppercase tracking-widest mb-10">
            {activeCategory ? activeCategory : "ALL FOOTWEAR PRODUCTS"}
          </h2>
          ...
          <FilterSortBar
            ...
            onClearFilters={() => {
              ...
              if (brandParam) router.push("/footwear");
            }}
            sortBy={sortBy}
            setSortBy={setSortBy}
          />
...
export default function FootwearPage() {
  return (
    <Suspense fallback={<div className="container mx-auto px-6 py-32 text-center text-muted-foreground">Loading Footwear Collection...</div>}>
      <FootwearPageContent />
    </Suspense>
  );
}
